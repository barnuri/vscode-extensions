...
export function findImportMatch<P extends { start: number; end: number; path: string }>(document: TextDocument, diagnostic: Diagnostic, fileImports: P[]) {
    const offset = document.offsetAt(diagnostic.range.start);
    return fileImports.find(i => i.start <= offset && i.end >= offset);
}

export async function removeUnusedImportChanges<C extends Change>(editor: TextEditor, changes: C[], getNewLineFromChange: (change: C) => string) {
    ...
    for (const change of changes) {
        const newLine = getNewLineFromChange(change);
    ...
    await editor.edit(builder => {
        ...
        builder.replace(new Range(document.positionAt(0), document.positionAt(oldTextEnd + endOffsetModifier)), newText);
    });
 ...
 */
type Change = {
    match: { start: number; end: number; path: string };
};
