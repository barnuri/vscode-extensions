...
class PoloniexBase(object):
            ...
            for delay in _chain(retryDelays, [None]):
                ...
                # we need to try again
                    ...
                    if delay is None:
                        ...
                        raise RetryException("retryDelays exhausted " + str(problem))
                    else:
    ...
    def __call__(self, command, args={}):
        ...
        if cmdType == "Private":
            ...
            sign = _new(
                self.secret.encode("utf-8"), _urlencode(args).encode("utf-8"), _sha512
            )
    ...
    def _checkCmd(self, command):
        ...
        if command in PRIVATE_COMMANDS:
            ...
            if not self.key or not self.secret:
                raise PoloniexError(
                    "An Api Key and Secret needed for command '%s'" % command
                )
            return "Private"
    ...
    def _handleReturned(self, data):
        ...
        if hasattr(data, "status_code") and (
            data.status_code == 502
            or data.status_code == 504
            or data.status_code in range(520, 527, 1)
        ):
            # log and try again
        ...
        try:
            if not self.jsonNums:
                ...
                out = _loads(
                    data.text, parse_float=self.jsonNums, parse_int=self.jsonNums
                )
        except:
            self.logger.error(data.text)
    ...
    def marketTradeHist(self, currencyPair, start=False, end=False):
        ...
        ret = self.session.get(
            "https://poloniex.com/public?" + _urlencode(args), timeout=self.timeout
        )
        # decode json
        return self._handleReturned(ret)
...
class Poloniex(PoloniexBase):
    ...
    def returnOrderBook(self, currencyPair="all", depth=20):
        ...
        return self.__call__(
            "returnOrderBook",
            {"currencyPair": str(currencyPair).upper(), "depth": str(depth)},
        )
    ...
    def returnLoanOrders(self, currency):
        ...
        return self.__call__("returnLoanOrders", {"currency": str(currency).upper()})
    ...
    def returnOpenOrders(self, currencyPair="all"):
        ...
        return self.__call__(
            "returnOpenOrders", {"currencyPair": str(currencyPair).upper()}
        )
    ...
    def returnOrderTrades(self, orderNumber):
        ...
        an error. """
        return self.__call__("returnOrderTrades", {"orderNumber": str(orderNumber)})
    ...
    def returnOrderStatus(self, orderNumber):
        ...
        return self.__call__("returnOrderStatus", {"orderNumber": str(orderNumber)})
    ...
    def cancelAllOrders(self, currencyPair=None):
        ...
        return self.__call__(
            "cancelAllOrders", {"currencyPair": str(currencyPair).upper()}
        )
    ...
    def withdraw(
        self, currency, amount, address, paymentId=False, currencyToWithdrawAs=False
    ):
    ...
    def transferBalance(
        self, currency, amount, fromAccount, toAccount, confirmed=False
    ):
    ...
    def getMarginPosition(self, currencyPair="all"):
        ...
        return self.__call__(
            "getMarginPosition", {"currencyPair": str(currencyPair).upper()}
        )
    ...
    def createLoanOffer(self, currency, amount, lendingRate, autoRenew=0, duration=2):
    ...
    def cancelLoanOffer(self, orderNumber):
        ...
        return self.__call__("cancelLoanOffer", {"orderNumber": str(orderNumber)})
    ...
    def toggleAutoRenew(self, orderNumber):
        ...
        return self.__call__("toggleAutoRenew", {"orderNumber": str(orderNumber)})
...
class PoloniexSocketed(Poloniex):
    ...
    def __init__(
        self, key=None, secret=None, subscribe={}, start=False, *args, **kwargs
    ):
        ...
        self.channels = {
            ...
            "ticker": {"id": "1002"},
            "24hvolume": {"id": "1003"},
            "heartbeat": {"id": "1010", "callback": self.on_heartbeat},
        }
    ...
    def _getChannelName(self, id):
        return next(
            (chan for chan in self.channels if self.channels[chan]["id"] == id), False
        )
    ...
    def on_message(self, data):
        if not self.jsonNums:
            ...
            message = _loads(data, parse_float=self.jsonNums, parse_int=self.jsonNums)
        # catch errors
        if "error" in message:
    ...
    def subscribe(self, chan, callback=None):
        ...
        if chan == "account":
            ...
            sign = _new(
                self.secret.encode("utf-8"), payload_encoded.encode("utf-8"), _sha512
            )
            ...
            if callback:
                self.channels[chan]["callback"] = callback
            self.socket.send(
                _dumps({"command": "subscribe", "channel": self.channels[chan]["id"]})
            )
    ...
    def unsubscribe(self, chan):
        ...
        if chan == "account":
            ...
            self.channels[chan]["sub"] = False
            self.socket.send(
                _dumps({"command": "unsubscribe", "channel": self.channels[chan]["id"]})
            )
